...
""" copy of https://appsrv.cse.cuhk.edu.hk/~pjhe/Drain.py with small bugfixes """


class Logcluster:
	# class for log groups
...
class Drain:
	...
	def treeSearch(self, rn, seq):
		# INPUT : 
		# 	seq = a raw log row in array format
		# 	rn = tree
		# OUTPUT :
		#	retLogClust : node or none
		retLogClust = None
		...
		# get the node with the correct length
		parentn = rn.childD[seqLen]

		# go under the tree, and find the log group
		currentDepth = 1
		for token in seq:
		...
		# get the best log event from the log group
	...
	def addSeqToPrefixTree(self, rn, logClust):
		...
		# find or create first depth node
		...
		for token in logClust.logTemplate:
			...
			#Add current log group to the leaf node
			if currentDepth>=self.para.depth or currentDepth>seqLen:
	...
	def SeqDist(self, seq1, seq2):
		# INPUT:
		#	seq1: template
		#	seq2: raw log row in array format
		# OUTPUT:
		#	retVal: simmilarity
		#	numOfPar: number of *'s
	...
	def FastMatch(self, logClustL, seq):
		# INPUT:
		#		logClustL: log group
		#		seq: raw log row in array format
		# OUTPUT:
		#		the best fitting log event for the seq
		...
		# find the best fitting log event to the seq
	...
	def outputResult(self, logClustL):
		print('Save files')
		templateDataFrame = dict()

		idx = 1
		for logClust in logClustL:
			templateDataFrame[logClust] = []
			for logID in logClust.logIDL:
				templateDataFrame[logClust].append(logID)
			idx += 1

		np.save(self.para.savePath+'logEventList.npy', templateDataFrame)
